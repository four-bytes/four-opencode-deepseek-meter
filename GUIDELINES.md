...
- **Source:** `src/four-opencode-deepseek-meter.ts` (NOT `src/index.ts`)
...
```
src/
├── four-opencode-deepseek-meter.ts   # Plugin entry — registers server + TUI
├── tui.tsx                           # SolidJS sidebar component
└── server.test.ts                    # Bun-native tests
```
